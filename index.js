...
$(document).ready(function() {
	Object.keys(state_info).forEach(function(key){
		...
		var url ='https://api.forecast.io/forecast/3cabe419c670d779000e17cb5ced8bf3/' + value.lat + ',' + value.lng;
		$.ajax({url:url, dataType:"jsonp"}).then(function(data) {
			var result = data.currently.temperature;
			console.log(result);

			if (result <= 10){
				$(help_key).css('fill', 'blue');
				console.log(value.lat);
			}
			else if (result >= 11 && result <= 30){
				$(help_key).css('fill', 'red');
			}
			else if (result >= 31 && result <= 51){
				$(help_key).css('fill', 'green');
			}
			else if (result >= 51 && result <= 80){
				$(help_key).css('fill', 'purple');
			}
			else if (result > 80){
				$(help_key).css('fill', 'yellow');
			}
			else{
				$(help_key).css('fill', 'black');
			}
		})
